...
export function AssessmentDetailPage() {
  const { id = '' } = useParams();
  const { data: assessment, loading } = useApi(() => getAssessment(id), [id]);
    ...
    return (
      <div>
        ...
        <Card className="p-6 text-sm text-muted-foreground">
          This assessment no longer exists. It may have been created in a previous session — mock data resets on
          reload.
        </Card>
      </div>
    );
