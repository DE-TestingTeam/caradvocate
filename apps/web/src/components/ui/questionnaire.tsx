...
/**
 * shadcn/ui `questionnaire` (https://ui.shadcn.com/docs/components/base/questionnaire), taken as
 * published -- the `radix` base, part for part, with its own class names intact.
 *
 * WHY IT IS COPIED IN BY HAND. `shadcn add questionnaire` cannot fetch it: the docs are live but
 * the registry item 404s for both released styles. The behaviour is not here in any case -- step
 * state, validation, progress and keyboard shortcuts all live in `@shadcn/react/questionnaire`.
 *
 * WHERE ITS LOOK COMES FROM. The `cn-*` classes, which shadcn ships as a stylesheet rather than as
 * utilities. That stylesheet is `src/styles/style-nova-questionnaire.css`, and a `style-nova`
 * wrapper is what switches it on -- see the comment at the top of that file. Nothing visual has
 * been substituted here, so a future `shadcn add questionnaire` should be close to a no-op.
 *
 * The two departures from the published file, both forced: `IconPlaceholder` is a component of the
 * docs site rather than of the registry, so the tick is a plain lucide `Check`; and the imports
 * point at this app's `@/` paths.
 */
...
/**
 * React 18 does not know the `inert` attribute, and the primitive sets it on every nav button that
 * is off screen -- which logs a warning per button per step. What `inert` adds is belt to existing
 * braces: those buttons are `hidden` too, which already takes them out of the layout, the tab order
 * and the accessibility tree. Delete this when the app moves to React 19, where `inert` is a real
 * prop.
 *
 * The primitive sets `inert` on the inactive QUESTIONS as well, and that one cannot be filtered the
 * same way -- `Item` takes no `render` prop. Those warnings appear in a dev console only;
 * production React does not check attribute names.
 */
function withoutInert<T extends object>(renderProps: T): T {
  const { inert: _inert, ...rest } = renderProps as T & { inert?: unknown };
  return rest as T;
}
...
function QuestionnairePrevious({
  ...
  return (
    <QuestionnairePrimitive.Previous
      ...
      render={(renderProps) => <button {...withoutInert(renderProps)} />}
      {...props}
    >
      {children ?? 'Previous'}
    </QuestionnairePrimitive.Previous>
